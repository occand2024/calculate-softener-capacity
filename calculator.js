...
\f0\fs24 \cf0 (function () \{\
  // Tank catalog: Diameter \'d7 Height (inch) \uc0\u8594  Approx resin volume (cu ft)\
  // Values marked "~" are typical estimates; adjust to your specs if needed.\
  const tanks = [\
    \{ label: '10" \'d7 48" (Overall)',  dia: 10, h: 48, cuft: 1.0 \},\
    \{ label: '12" \'d7 52" (Overall)',  dia: 12, h: 52, cuft: 2.0 \},\
    \{ label: '14" \'d7 65" (Overall)',  dia: 14, h: 65, cuft: 3.0 \},\
    \{ label: '16" \'d7 65" (Overall)',  dia: 16, h: 65, cuft: 4.0 \},     // ~4 ft\'b3 typical\
    \{ label: '21" \'d7 62" (Overall)',  dia: 21, h: 62, cuft: 7.0 \},     // ~7 ft\'b3 typical\
    \{ label: '24" \'d7 60" (Straight Wall)', dia: 24, h: 60, cuft: 10.0 \},\
    \{ label: '30" \'d7 60" (Straight Wall)', dia: 30, h: 60, cuft: 15.0 \},\
    \{ label: '36" \'d7 60" (Straight Wall)', dia: 36, h: 60, cuft: 20.0 \},\
    \{ label: '42" \'d7 60" (Straight Wall)', dia: 42, h: 60, cuft: 21.0 \}, // ~21 ft\'b3 typical\
    \{ label: '42" \'d7 72" (Straight Wall)', dia: 42, h: 72, cuft: 25.0 \},\
    \{ label: '48" \'d7 60" (Straight Wall)', dia: 48, h: 60, cuft: 35.0 \},\
    \{ label: '48" \'d7 72" (Straight Wall)', dia: 48, h: 72, cuft: 42.0 \}, // ~42 ft\'b3 typical\
    \{ label: '54" \'d7 60" (Straight Wall)', dia: 54, h: 60, cuft: 50.0 \},\
    \{ label: '60" \'d7 60" (Straight Wall)', dia: 60, h: 60, cuft: 65.0 \},\
  ];\
\
...
\
  function initTankSelect() \{\
    ...
    tanks.forEach((t, i) => \{\
      ...
      opt.value = String(i);\
      opt.textContent = `$\{t.label\} \'97 $\{t.cuft\} ft\'b3`;\
      sel.appendChild(opt);\
    \});\
    // Set initial resin ft\'b3 from first option\
...
\
    const capPerFt3 = num(el('saltPreset').value); // grains per ft\'b3 preset\
...
\
    ...
    if (!hardnessGpg || hardnessGpg <= 0 || !dailyUse || dailyUse <= 0 || !perVesselResin || perVesselResin <= 0 || !capPerFt3 || capPerFt3 <= 0) \{\
      resultsDiv.innerHTML = `<div class="muted">Please enter valid positive numbers for hardness, daily use, resin ft\'b3, and capacity preset.</div>`;\
      return;\
    \}\
\
    ...
    resultsDiv.innerHTML = `\
      <table>\
        <tbody>\
          ...
          <tr>\
            <th>Resin Volume</th>\
            <td>$\{fmt(perVesselResin, 2)\} ft\'b3 per vessel \'d7 $\{vesselCount\} = <strong>$\{fmt(totalResinCuFt, 2)\} ft\'b3 total</strong></td>\
          </tr>\
          <tr>\
            <th>Capacity Curve Selected</th>\
            <td>$\{fmt(capPerFt3)\} grains/ft\'b3 $\{\
              saltDose ? `<span class="badge">~$\{fmt(saltDose)\} lb/ft\'b3</span>` : ''\
            \}</td>\
          </tr>\
          <tr>\
            <th>Effective Hardness</th>\
            <td>$\{fmt(effHardness, 2)\} gpg <span class="muted">(= Hardness + 3\'d7Fe + 2\'d7Mn)</span></td>\
          </tr>\
          <tr>\
          ...
          <tr>\
            ...
            <td>$\{fmt(capacityGalSafe)\} gallons <span class="badge">\'d7 0.90</span></td>\
          </tr>\
          <tr>\
...
\
  ...
  document.addEventListener('DOMContentLoaded', () => \{\
    ...
    tanks.forEach((t, i) => \{\
      ...
      sel.appendChild(opt);\
    \});\
    // set default resin ft\'b3 from first option\
